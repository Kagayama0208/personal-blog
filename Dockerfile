...
# Migration runner: keeps the full source + node_modules (and the `payload` CLI)
# so it can load the config and apply committed migrations. The slim `runner`
# stage below cannot do this. Build with `--target migrator` and run as the K8s
# migrate Job (`payload migrate`) BEFORE rolling out the app.
